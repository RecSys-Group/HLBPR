...
class BPR(object):
    def __init__(self, D, args,users, items, trainfile, testfile, pu, qi, similar_file):
        self.D = D
    ...
    def train(self, data, sampler, num_iters):
        ...
        t.to_csv('ourbprnn50pu')
        t = pd.DataFrame(self.item_factors)
        t.to_csv('ourbprnn50qi')
        t = pd.DataFrame(self.item_bias)
        t.to_csv('ourbprnn50b')
    ...
    def create_loss_samples(self):
        ...
        sampler = UniformUserUniformItem(True, self.simifile, self.trainfile)
...
class Sampler(object):
    def __init__(self, sample_negative_items_empirically, similar_file, trainfile):
    ...
    def sample_k_item(self, items_list, uid, score=0):
        j = self.random_item()
        while self.user_item_pref(uid, j) == 0.0:
            j = self.random_item()
        return j

    def sample_j_item(self, items_list, uid, score=0):
        j = self.random_item()
        while j in items_list or self.user_item_pref(uid, j) > 0.0:
            j = self.random_item()
        return j
...
def test_model(data, users, items, trainfile, testfile, pu, qi, similar_file, learning_rate, b_regularization=1, u_regularization=0.01, p_regularization=0.01,
               n_regularization=0.01, factor=200, iter=100):
    args = BPRArgs()
    ...
    num_factors = factor
    model = BPR(num_factors, args, users, items, trainfile, testfile, pu, qi, similar_file)
    sample_negative_items_empirically = True
    sampler = UniformPairWithoutReplacement(sample_negative_items_empirically, similar_file, trainfile)
    num_iters = iter
    model.train(data, sampler, num_iters)

def start(users, items, trainfile, testfile, pu, qi, similar_file):
    data = gen_input_data(trainfile)
    test_model(data, users, items, trainfile, testfile, pu, qi, similar_file, 0.001, 0.0, 0.002, 0.0003, 0.0003)
