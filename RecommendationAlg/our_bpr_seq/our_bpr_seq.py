...
class BPR(object):
    def __init__(self, D, args,users, items, trainfile, testfile, pu, qi, similar_file, users_of_pattern, new_format_seq):
    ...
    def train(self, data, sampler, num_iters):
        ...
        t.to_csv('ourbprseq50pu')
        t = pd.DataFrame(self.item_factors)
        t.to_csv('ourbprseq50qi')
        t = pd.DataFrame(self.item_bias)
        t.to_csv('ourbprseq50b')
    ...
    def create_loss_samples(self):
        ...
        sampler = UniformUserUniformItem(True, self.simifile, self.trainfile, self.users_of_pattern, self.new_format_seq)
...
class Sampler(object):
    def __init__(self, sample_negative_items_empirically, similar_file, trainfile, users_of_pattern, new_format_seq):
        ...
        self.trainfile = trainfile
        self.simi = pd.read_csv(self.similar_file)
    ...
    def cos_sim(self, a, b):
        t1 = np.sum(a ** 2)
        t2 = np.sum(b ** 2)
        return np.sum(a * b) / (np.sqrt(t1) * np.sqrt(t2))

    def user_map(self, uid):
        file = 'error_map'
        data = pd.read_csv(file)
        from_indexes = data.iloc[:, 2].values
        index = np.argwhere(np.array(from_indexes) == uid)
        return index[0][0]
    ...
    def user_item_pref_SEQ(self, uid, pid):
        ...
        dictarray = [dict(eval(i)) for i in np.array(self.simi.iloc[uid])[1:] if isinstance(i, str)]
    ...
    def sample_k_item(self, items_list, uid, score=0):
        j = self.random_item()
        a = 0
        while self.user_item_pref_SEQ(uid, j) == 0.0 and a <= 100:
            j = self.random_item()
            a += 1
        if a > 100:
            return 10000
    ...
    def sample_j_item(self, items_list, uid, score=0):
        j = self.random_item()
        while j in items_list or self.user_item_pref_SEQ(uid, j) > 0.0:
            j = self.random_item()
        return j
...
def test_model(data, users, items, trainfile, testfile, pu, qi, similar_file, users_of_pattern, new_format_seq, learning_rate, b_regularization=1, u_regularization=0.01, p_regularization=0.01,
               n_regularization=0.01, factor=150, iter=100):
    args = BPRArgs()
    ...
    num_factors = factor
    model = BPR(num_factors, args, users, items, trainfile, testfile, pu, qi, similar_file, users_of_pattern, new_format_seq)
    sample_negative_items_empirically = True
    sampler = UniformPairWithoutReplacement(sample_negative_items_empirically, similar_file, trainfile, users_of_pattern, new_format_seq)
    num_iters = iter
    model.train(data, sampler, num_iters)

def start(users, items, trainfile, testfile, pu, qi, similar_file, users_of_pattern, new_format_seq):
    data = gen_input_data(trainfile)
    test_model(data, users, items, trainfile, testfile, pu, qi, similar_file, users_of_pattern, new_format_seq, 0.005, 0.0, 0.002, 0.0003, 0.0003)
